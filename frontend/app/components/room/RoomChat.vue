 ...
 */
import type { RoomCapabilitiesView, RoomChatMessageView } from '#shared/api/types'
...
function startCooldown() {
  const slow = props.capabilities.slowModeSeconds
  if (slow <= 0) return
  cooldown.value = slow
...
const sendDisabled = computed(() => {
  if (!props.capabilities.canSendMessages) return true
  if (props.capabilities.isMuted) return true
...
const placeholder = computed(() => {
  if (props.capabilities.isMuted) return 'Сізге уақытша жазуға болмайды'
  if (!props.capabilities.canSendMessages) return 'Чат тек қарап отыруға ашық'
    ...
    >
      ...
      <ul v-else class="space-y-3">
        <li
          ...
          :class="{ 'opacity-70': m.type === 'SYSTEM' }"
        >
          <div
            ...
            :class="{ 'bg-warning-100 text-warning-700': m.type === 'ADMIN', 'bg-slate-100 text-slate-500': m.type === 'SYSTEM' }"
          >
    ...
    <footer class="border-t border-slate-100 bg-slate-50 px-3 py-3">
      <form class="flex items-center gap-2" @submit.prevent="handleSend">
        <input
          ...
          type="text"
          maxlength="500"
          :disabled="!capabilities.canSendMessages || capabilities.isMuted"
